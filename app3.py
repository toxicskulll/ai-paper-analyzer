...
def enhanced_summarize_text(text: str, style: str = "comprehensive", metadata: dict = None, model: str = MODEL_NAME) -> str:
    """Enhanced summarization with better prompts including metadata and improved subprocess handling."""
    ...
    prompt_templates = {
        "comprehensive": f"""{context_info}Analyze the following research paper content and provide a comprehensive summary:\n\n{text}""",
        "bullet-point": f"""{context_info}Summarize the following content in bullet points:\n\n{text}""",
        "paragraph": f"""{context_info}Write a paragraph summary of the following:\n\n{text}""",
        "detailed": f"""{context_info}Write a detailed structured summary of the following:\n\n{text}"""
    }
...
# def enhanced_summarize_text(text: str, style: str = "comprehensive", metadata: dict = None, model: str = MODEL_NAME) -> str:
#     """Enhanced summarization with better prompts including metadata"""
...
#         "bullet-point": f"""{context_info}Create a structured bullet-point analysis:
...
#         "paragraph": f"""{context_info}Write a comprehensive paragraph summary covering the research objective, methodology, key findings, contributions, limitations, and future work.

# Content: {text}""",

#         "detailed": f"""{context_info}Provide a detailed structured analysis with clear sections for objectives, methodology, contributions, results, limitations, and future work.
...
#             else:
#                 logging.warning(f"Summarization attempt {attempt + 1} failed")
            ...
            for i, paper in enumerate(st.session_state.papers_data):
                with st.expander(f"📄 {paper['title'][:100]}..." if len(paper['title']) > 100 else f"📄 {paper['title']}", expanded=i==0):
                    ...
                    with col2:
                        ...
                        if st.button(f"📄 Generate PDF Report", key=f"pdf_{i}"):
                            st.info("PDF generation feature will be implemented")
